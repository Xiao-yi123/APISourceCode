...
<title>知乎最新的新闻列表 - <?php echo $aik['sitename']; ?></title>
</head>
<style type="text/css">
...
<div style="box-shadow: 5px 5px 25px 0 rgba(46,61,73,.2);border-radius:15px;font-size:13px;width:950px;font-family:微软雅黑,'Helvetica Neue',Arial,sans-serif;margin:10px auto 0px;border:0px solid #eee;max-width:100%;">
    <div style="width:100%;background-color: #3174ed;background-image: linear-gradient(90deg, #3174ed 0%, #FA8BFF 35%, #3fd9fb 88%);color:#FFFFFF;border-radius:15px 15px 0 0;">
        <h2 style="font-size:15px;word-break:break-all;padding:20px 32px;margin:0;text-align:center">知乎最新的新闻列表 - API</h2>
    </div>
	...
	直接返回josn格式<br/>	</div>
		<hr>
		# 备注 #<br/>
		将返回的ID号进行解析即可得到详情信息<br/>例如："id":9720670（则是该文章的id号）<br/><?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/api/lanzou/apijx.php?id=<br/>
		<hr>
		# 获取最新新闻示例 #<br/>
		<a style="color:#fd4b5c;text-decoration:none；" target="_blank"><?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/api/lanzou/api.php</a><br>
		<hr>
		# 获取该新闻文章详情示例 #<br/>根据上个 API 接口的新闻 id,返回该新闻的详细信息<br/>
        <a style="color:#fd4b5c;text-decoration:none；" target="_blank"><?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/api/lanzou/apijx.php?id=9720670</a>
		</h3>
          </div>
</div>
<script>
