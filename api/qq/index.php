...
<title>获取QQ昵称和头像 - <?php echo $aik['sitename']; ?></title>
</head>
<style type="text/css">
...
<div style="box-shadow: 5px 5px 25px 0 rgba(46,61,73,.2);border-radius:15px;font-size:13px;width:950px;font-family:微软雅黑,'Helvetica Neue',Arial,sans-serif;margin:10px auto 0px;border:0px solid #eee;max-width:100%;">
    ...
    <div style="margin:0px auto;width:90%">
		...
		<hr>
		# 请求地址 #<br/>
		<a style="color:#fd4b5c;text-decoration:none;"><?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/api/qq/api.php</a><br/>
		<a style="color:#fd4b5c;text-decoration:none;"><?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/api/qq/qq.php</a><br/>
		# 参数 #<br/>
		<li>qq //要查询的QQ号(必填)</li> 
		<hr>
		# 返回数据 #<br/>
		...
		{"code":1,"imgurl":"https://q.qlogo.cn/headimg_dl?dst_uin=2248186422&spec=100","name":"智云"}<br/>
        {"code":-1,"msg":"Error"}<br/>
		</div>
		<hr>
		# 备注 #<br/>
		...
		<hr>
		# 示例 #<br/>
		<a style="color:#fd4b5c;text-decoration:none；" target="_blank">1.头像获取<?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/api/qq/api.php?qq=2248186422</a><p><a style="color:#fd4b5c;text-decoration:none；" target="_blank">2.头像及昵称获取<?php echo filter_input(INPUT_SERVER, "SERVER_NAME"); ?>/api/qq/qq.php?qq=2248186422</a>
		</h3>
          </div>
</div>
<script>
