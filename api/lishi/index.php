...
//获取输出数量(可以get调整)
$num = @$_GET['num'] ? ($_GET['num']>$countnum ? $countnum :$_GET['num']) : $countnum;
$arr = array('code'=>'200','day'=>$today,'content'=>[]);
for ($x=0; $x<=$num; $x++) {
    $title = match_chinese(strip_tags($json[$month][$month.$day][$x]['title']));
    $arr['content'][] .= $title;
}
